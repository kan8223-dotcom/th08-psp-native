...
#if defined(PSP)
// Presentation-only ownership for the two strings generated by
// Supervisor::CalculateFps.  The scope is deliberately separate from
// AsciiManager's retail layout and saves/restores its predecessor so a nested
// diagnostic caller cannot relabel surrounding strings.
enum class PspAsciiRenderOwner : u8
{
    Game = 0,
    FpsCounter,
    ReplayFpsDiagnostic,
...
class PspAsciiRenderOwnerScope
{
public:
    explicit PspAsciiRenderOwnerScope(PspAsciiRenderOwner owner);
    ~PspAsciiRenderOwnerScope();

    PspAsciiRenderOwnerScope(const PspAsciiRenderOwnerScope &) = delete;
    PspAsciiRenderOwnerScope &operator=(
        const PspAsciiRenderOwnerScope &) = delete;

private:
    PspAsciiRenderOwner previousOwner_;
};
#endif
